...
<div class="row">
    <div class="col-lg-12">
        ...
        <div class="card-header">
            <h4 style="margin-right: 28%;">Data Teknologi Tepat Guna, Produk, Karya Seni, Rekayasa Sosial</h4>
        ...
        <div class="card-body">
            ...
            <div class="table-responsive">
                <table id="dataTable" class="table table-hover">
                    ...
                    <tbody>
                        @php $no=1 @endphp
                        @foreach($data_teknologi_tepat_guna as $data)
                        <tr>
                            <td>{{$no++ }}</td>
                            <td>{{$data->luaran_penelitian_pkm }}</td>
                            <td>{{$data->tahun }}</td>
                            <td>{{$data->keterangan }}</td>
                            <td><a href="{{route('prodi-file_download_file_teknologi_tepat_guna',$data->id)}}"><button class="btn btn-info ">Download</button></a></td>
        ...
        <div class="card">
            <div class="card-body">
                ...
                <p style="font-size: 10px;">
                    ...
                    3. Unggah File dengan Format (jpg/png/word/pdf/rar/zip) dan Link Drive Bukti Teknologi Tepat Guna, Produk
                    (Produk Terstandarisasi, Produk Tersertifikasi), Karya Seni, atau Rekayasa Sosial
                </p>
            </div>
        </div>
    </div>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Tambah Data HKI (Paten, Paten Sederhana)</h5>

            </div>
            <div class="modal-body">
                <form method="post" action="{{route('prodi_kinerja_dosen_teknologi_tepat_guna_add')}}" enctype="multipart/form-data">
...
<script type="text/javascript">
    function deleteData(id) {
        var id = id;
        var url = '{{route("prodi_kinerja_dosen_teknologi_tepat_guna_delete", ":id") }}';
...
<script>
  $(document).ready(function() {
    ...
    $('#updateModalform').attr('action','prodi_kinerja_dosen_teknologi_tepat_guna_update/'+ data[7]);
