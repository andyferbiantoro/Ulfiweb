    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Tambah Data Produk/Jasa Mahasiswa yang Diadopsi oleh Industri/Masyarakat</h5>

            </div>
            <div class="modal-body">
                <form method="post" action="{{route('prodi_produk_jasa_mahasiswa_diadopsi_oleh_industri_masyarakat_add')}}" enctype="multipart/form-data">
...
<div id="updateModal" class="modal fade" role="dialog">
  ...
  <div class="modal-body">
    ...
    <div class="form-group">
        <label for="judul">Deskripsi Produk/Jasa</label>
        <textarea type="text" class="form-control" id="deskripsi_produk_update" name="deskripsi_produk" required=""> </textarea>
    </div>
...
<script type="text/javascript">
    function deleteData(id) {
        var id = id;
        var url = '{{route("prodi_produk_jasa_mahasiswa_diadopsi_oleh_industri_masyarakat_delete", ":id") }}';
...
<script>
  $(document).ready(function() {
    ...
    console.log(data);
    $('#nama_mahasiswa_update').val(data[1]);
    $('#nama_produk_update').val(data[2]);
    $('#deskripsi_produk_update').val(data[3]);
    $('#link_bukti_update').val(data[5]);
    $('#tahun_update').val(data[6]);

    $('#updateModalform').attr('action','prodi_produk_jasa_mahasiswa_diadopsi_oleh_industri_masyarakat_update/'+ data[8]);
